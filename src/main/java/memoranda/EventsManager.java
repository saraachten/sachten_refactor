...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	//composed of multiple nested if statements that were increasing the complexity
	//to decrease complexity by combining if statements so multiple requirments checks at same time
	//place in bad smell category of long method
		...
		for (int i = 0; i < reps.size(); i++) {
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
				if (ev.getRepeat() == REPEAT_DAILY) {
					...
					int ns =
						ev.getStartDate().getCalendar().get(Calendar.DAY_OF_YEAR);
					if ((n - ns) % ev.getPeriod() == 0)
						v.add(ev);
				} else if (ev.getRepeat() == REPEAT_WEEKLY || ev.getRepeat() == REPEAT_MONTHLY) {
					if (date.getCalendar().get(Calendar.DAY_OF_WEEK) == ev.getPeriod() ||
					        date.getCalendar().get(Calendar.DAY_OF_MONTH) == ev.getPeriod())
						v.add(ev);
				} else if (ev.getRepeat() == REPEAT_YEARLY &&
				        date.getCalendar().get(Calendar.DAY_OF_YEAR) == ev.getPeriod()) {
						v.add(ev);
				}
			}
		}
